...
const updateLogin = (request, response) => {
  pool.query("SELECT * FROM users WHERE uid = $1", [request.signedCookies.info.uid], (error, user_log) => {
    ...
    if (request.body.name && request.body.name != user_log.rows[0].name) {
      pool.query(
        ...
        [request.body.name, request.signedCookies.info.uid],
        (error, results) => {
          if (error) {
            throw error;
          }
    ...
    if (request.body.surname && request.body.surname != user_log.rows[0].firstname) {
      pool.query(
        "UPDATE users SET firstname = $1 WHERE uid = $2",
        [request.body.surname, request.signedCookies.info.uid],
        (error, results) => {
          if (error) {
            throw error;
          }
      ...
      pool.query("SELECT * FROM users WHERE login = $1", [request.body.login], (error, check_login) => {
        if (error) {
          ...
          if (check_login.rowCount === 0) {
            pool.query(
              "UPDATE users SET login = $1 WHERE uid = $2",
              [request.body.login, request.signedCookies.info.uid],
              (error, results) => {
                if (error) {
                  throw error;
                }
        ...
        if (error) {
          ...
          if (check_email.rowCount === 0) {
            pool.query(
              ...
              [request.body.email, request.signedCookies.info.uid],
              (error, results) => {
                if (error) {
                  throw error;
                }
    ...
    if (request.body.birthday && moment(request.body.birthday, "YYYY/MM/DD") != user_log.rows[0].birthday) {
      pool.query(
        "UPDATE users SET birthday = $1 WHERE uid = $2",
        [moment(request.body.birthday, "YYYY/MM/DD"), request.signedCookies.info.uid],
        (error, results) => {
          if (error) {
            throw error;
          }
    ...
    if (request.body.gender && request.body.gender.toLowerCase() != user_log.rows[0].gender) {
      pool.query(
        "UPDATE users SET gender = $1 WHERE uid = $2",
        [request.body.gender.toLowerCase(), request.signedCookies.info.uid],
        (error, results) => {
          if (error) {
            throw error;
          }
    ...
    if (
      request.body.sexual_orientation &&
      request.body.sexual_orientation.toLowerCase() != user_log.rows[0].sexual_orientation
    ) {
      pool.query(
        "UPDATE users SET sexual_orientation = $1 WHERE uid = $2",
        [request.body.sexual_orientation.toLowerCase(), request.signedCookies.info.uid],
        (error, results) => {
          if (error) {
            throw error;
          }
    ...
    if (request.body.description && request.body.description != user_log.rows[0].description) {
      pool.query(
        "UPDATE users SET description = $1 WHERE uid = $2",
        [request.body.description, request.signedCookies.info.uid],
        (error, results) => {
          if (error) {
            throw error;
          }
    ...
    if (request.body.location && request.body.location != user_log.rows[0].location) {
      pool.query(
        "UPDATE users SET country = $1 WHERE uid = $2",
        [request.body.location, request.signedCookies.info.uid],
        (error, results) => {
          if (error) {
            throw error;
          }
    ...
    if (request.body.addTag) {
      ...
      for (let i = 0; word[i]; i++) {
        pool.query(
          "INSERT INTO tag(uid, tag) VALUES ($1, $2)",
          [request.signedCookies.info.uid, word[i]],
          (error, results) => {
            if (error) {
              response.status(400);
            }
  ...
  if (response) {
    ...
    response.json({ info: "this email is already used" }).status(400);
  }
};
...
const deleteTag = (request, response) => {
  pool.query(
    "DELETE FROM tag WHERE uid = $1 AND tag = $2",
    [request.signedCookies.info.uid, request.body.tag],
    (error, results) => {
      if (error) {
        throw error;
      } else {
...
const deleteImage = (request, response) => {
  pool.query(
    "DELETE FROM img WHERE uid = $1 AND n_pic = $2",
    [request.signedCookies.info.uid, request.body.img],
    (error, results) => {
      if (error) {
        throw error;
      } else {
